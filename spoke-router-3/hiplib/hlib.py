...
class HIPLib():
    def __init__(self, config, completed_callback=None, on_closed = None):
        self.config = config;

        # Notifies the caller about completed HIP-BEX
        ...
        try:
            ...
            if hip_packet.get_packet_type() == HIP.HIP_I1_PACKET:
                ...
                if (end_time - start_time) > (2 << (puzzle_param.get_lifetime() - 32)):
                    ...
                    if sv.is_responder:
                        self.on_closed(sv.rhit, sv.ihit, sv.src, sv.dst)
                    else:
                        self.on_closed(sv.ihit, sv.rhit, sv.src, sv.dst)
                    return [];
    ...
    def trigger_bex(self, ihit, rhit, src_str, dst_str):
        try:
            ...
            if hip_state.is_unassociated() or hip_state.is_closing() or hip_state.is_closed():
                ...
                response = (True, bytearray(ipv4_packet.get_buffer()), (dst_str.strip(), 0))
